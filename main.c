...
//=================================================================================
    ...
    CpuTimer1Regs.PRD.all =  fire_angle;    // задействую аппаратный таймер дл€ измерени€ угла тиристора
    ...
    if (INCR_BUILD == 3)
    {
        ...
        else        //
        {
            if(!fire_angle_min)
            {
                ...
                if(fire_angle_count ==0 )     //урежение на пор€док до 100 мс
                {
                    if(fire_angle > 110)
...
//-----------------------------------------
{
    ...
    if (run_flag == 1) //If soft-start is over and PFC running normally
    {
        ...
        // если PFC вышел на режим и VBUS достигло 400 ¬, плавно поднимаем насыщение  V регул€тора инвертора
