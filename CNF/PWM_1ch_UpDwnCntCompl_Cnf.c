...
void PWM_1ch_UpDwnCntCompl_CNF(int16 n, Uint16 period, int16 mode, int16 phase, int16 DB ) {
	...
	(*ePWM[n]).DBCTL.bit.OUT_MODE = 3;					// Dead-band is fully enabled
	//(*ePWM[n]).DBCTL.bit.POLSEL = 1;					// Both EPWMxA and EPWMxB are inverted
	(*ePWM[n]).DBCTL.bit.POLSEL = 0;					// Both EPWMxA and EPWMxB are not inverted
	(*ePWM[n]).DBCTL.bit.IN_MODE = 0;
	(*ePWM[n]).DBRED = DB;
